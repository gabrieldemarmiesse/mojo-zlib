...
def test_decompression_object_attributes():
    ...
    # Create test data
    test_data = "Hello, World! This is a test for decompression attributes.".as_bytes()
    ...
    assert_equal(len(result), len(test_data), "Decompressed data length should match original")
...
def test_unconsumed_tail_partial_decompress():
    ...
    if len(result) == 0:
        ...
        assert_equal(len(unconsumed), partial_size, "unconsumed_tail should contain all fed data when no decompression occurs")
    else:
